-- migrate:up
-- Setup users and permissions for config database
...
DECLARE
    ...
    read_only_users TEXT[] := ARRAY[]::TEXT[];
    username TEXT;
BEGIN
    ...
    FOREACH username IN ARRAY read_write_users LOOP
        ...
        END IF;
        EXECUTE format('GRANT ALL PRIVILEGES ON DATABASE config TO %I', username);
    ...
    FOREACH username IN ARRAY read_only_users LOOP
        IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = username) THEN
            EXECUTE format('CREATE ROLE %I WITH LOGIN', username);
        END IF;
        EXECUTE format('GRANT CONNECT ON DATABASE config TO %I', username);
...
DO $$
DECLARE
    read_write_users TEXT[] := ARRAY['grpcserver'];
    read_only_users TEXT[] := ARRAY[]::TEXT[];
    username TEXT;
BEGIN
    ...
    FOREACH username IN ARRAY read_only_users LOOP
        ...
        EXECUTE format('REVOKE CONNECT ON DATABASE config FROM %I', username);
    ...
    FOREACH username IN ARRAY read_write_users LOOP
        ...
        EXECUTE format('REVOKE ALL PRIVILEGES ON DATABASE config FROM %I', username);
        EXECUTE format('DROP ROLE IF EXISTS %I', username);
    END LOOP;
END
$$;
